...
public class Snake extends Applet implements ActionListener {
	...
	int directionX[] = new int[SnakeLength]; // X Direction each oval is going
												// (RIGHT, LEFT)
	int directionY[] = new int[SnakeLength]; // Y Direction each oval is going
												// (DOWN, UP)
	...
	public void paint(Graphics g) {
		...
		for (i = 0; i < SnakeLength; i++) {
			g.setColor(SnakeColor[i]); // Set the color to the color previously
										// set
			g.fillOval(x[i], y[i], SnakeWidth, SnakeWidth); // Draw the oval

			if (x[i] >= getWidth() - SnakeWidth) // Check if the ball is at the
													// edge of the screen
			...
			if (y[i] >= getHeight() - SnakeWidth) // Check if the ball is at the
													// edge of the screen
			...
			if (directionX[i] == 0) // Check if the ball is going RIGHT
				x[i] += SnakeSpeed[i]; // Add the value of the current balls
										// speed to the current balls x position
										// (Makes the ball move to the right)
			if (directionX[i] == 1) // Check if the ball is going LEFT
				x[i] -= SnakeSpeed[i]; // Add the value of the current balls
										// speed to the current balls x position
										// (Makes the ball move to the left)
			if (directionY[i] == 0) // Check if the ball is going DOWN
				y[i] += SnakeSpeed[i]; // Add the value of the current balls
										// speed to the current balls y position
										// (Makes the ball move to the down)
			if (directionY[i] == 1) // Check if the ball is going UP
				y[i] -= SnakeSpeed[i]; // Add the value of the current balls
										// speed to the current balls y position
										// (Makes the ball move to the up)

			if (x[i] <= 0) // Check if the ball is at the edge of the screen
			{
				setBackground(Color.black);
				SnakeColor[i] = new Color((int) (Math.random() * 255), (int) (Math.random() * 255),
						(int) (Math.random() * 255));
			} // Random ball color
	...
	public void actionPerformed(ActionEvent e) // Called each time the timers
												// delay is up
	{
